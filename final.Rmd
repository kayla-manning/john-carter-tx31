...
{
  dw <- read_csv("data/all_dwnom.csv")
  house_elxn <- read_csv("data/1976-2018-house3.csv") %>% 
    select(year, state_po, office, district, candidate, party,
           candidatevotes, totalvotes, stage) %>% 
    rename(state = state_po)
  pres_elxn <- read_csv("data/countypres_2000-2016.csv") %>% 
    select(-c(state, FIPS, version)) %>% 
    rename(state = state_po)
  religion <- read_csv("http://acme.highpoint.edu/~msetzler/Congressional_District_Religiosity_Dataset/ReligEstimatesByCD_07_22_2018.csv")
  jc <- read_excel("../exercises/data/jc_voting.xlsx", skip = 3) %>% 
    clean_names() %>% 
    rename(scores = voting_scores_carter_john_2003) %>% 
    drop_na(year)
  party <- read_dta("../exercises/data/ex3_party.dta")
}
...
```{r religion, fig.width=7, fig.height=4}

# getting TX average across all congressional districts & then normalizing

tx_avg <- religion %>% 
  clean_names() %>% 
  select(-c("any_cong", "no_cong")) %>% 
  filter(yr_elected == 2018,
         stateabbr == "TX") %>% 
  select(-c(stcode, cong_session, yr_elected, c_dnum)) %>% 
  summarise(across(where(is.numeric), mean)) %>% 
  rename_with(~paste0(., "_tx")) %>% 
  mutate(total = rowSums(.)) %>% 
  mutate(across(everything(), function(x) x/total*100)) %>% 
  select(-total)
...
tx31_avg <- religion %>% 
  ...
  select(-c(stcode, cong_session, yr_elected, c_dnum,
            ab_bcode, stateabbr)) %>% 
  rename_with(~paste0(., "_tx31")) %>% 
  mutate(total = rowSums(.)) %>% 
  mutate(across(everything(), function(x) x/total*100)) %>% 
  select(-total)

# plot with overall numbers

overall <- religion %>% 
  clean_names() %>% 
  select(stateabbr, yr_elected, c_dnum, any_cong, no_cong) %>% 
  filter(yr_elected == 2018,
         stateabbr == "TX") %>% 
  mutate(tx31 = ifelse(c_dnum == 31, TRUE, FALSE)) %>% 
  group_by(tx31) %>% 
  summarise(any_cong = mean(any_cong) / 100,
            no_cong = mean(no_cong) / 100) %>% 
  pivot_longer(2:3, names_to = "congregation") %>% 
  ggplot(aes(tx31, value, fill = congregation)) +
  geom_col() +
  geom_text(aes(label = paste0(round(value * 100, 1), "%")), 
            position = position_stack(vjust = 0.5)) +
  theme_hodp() +
  scale_x_discrete(labels = c("Average of TX Districts",
                              "TX-31")) +
  labs(x = "",
       y = "Percentage of population",
       fill = "Belongs to a \ncongregation?",
       title = "A lower-than-average proportion of TX-31 residents \nbelong to a religious congregation") +
  scale_fill_manual(breaks = c("any_cong", "no_cong"),
                    labels = c("Yes", "No"),
                    values = c(primary[4], primary[5])) +
  scale_y_continuous(labels = scales::percent)

# plot with congregation breakdown

breakdown <- bind_cols(tx_avg, tx31_avg) %>% 
  pivot_longer(everything(), names_to = "relig") %>% 
  mutate(geo = ifelse(str_detect(relig, "_tx$"), "tx", "tx31"),
         relig = ifelse(str_detect(relig, "_tx31$"), 
                        str_remove(relig, "_tx31"), 
                        str_remove(relig, "_tx")),
         value = value / 100) %>%
  ggplot(aes(geo, value, fill = relig)) +
  geom_col() + 
  geom_text(aes(label = paste0(round(value * 100, 1), "%")), 
            position = position_stack(vjust = 0.5)) +
  theme_hodp() +
  labs(x = "",
       y = "Percentage of Population",
       fill = "",
       title = "Among those affiliated with a congregation, TX-31 has a greater \nshare of Protestants compared to TX overall",
       caption = "*The data groups Black Protestant, LDS (Mormon), \nand Jehovah's Witnesses congregations \ninto the Other category. ") +
  coord_flip() +
  scale_x_discrete(breaks = c("tx31", "tx"),
                   labels = c("TX-31", "TX")) +
  scale_fill_manual(labels = c("Catholic",
                               "Evangelical Protestant",
                               "Mainline Protestant",
                               "Other*"),
                      values = c(primary[1], primary[3:5])) +
  scale_y_continuous(labels = scales::percent)
...
```{r election_history, fig.width=6, fig.height=4}

# want to create a plot that shows how the district has voted in house &
# presidential races over time

# TX-31 had 89% of Bell County and 100% of Williamson County in 2012, so I will
# just use those counties for the presidential data... not ideal since district
# is redrawn, but it will give us a crude sense of the relationship

# I will start off by getting the presidential data ready... note that I am
# manually calculating Trump's vote share for Bell County in 2020, using vote
# counts from county clerk websites (it was just easier to do this than to
# download a whole other dataset for a single row)


tx31_pres <- pres_elxn %>% 
  filter(state == "TX",
         county %in% c("Bell", "Williamson")) %>% 
  mutate(party = ifelse(is.na(party), "other", party)) %>% 
  filter(party == "republican") %>% 
  group_by(year, office) %>% 
  summarise(rvoteshare = sum(candidatevotes) / sum(totalvotes),
            .groups = "drop") %>% 
  bind_rows(tibble(year = 2020,
                   rvoteshare = (67893 + 139729) / 
                     (127690 + 290168),
                   office = "President"))

# now I am joining the presidential returns for the district with the returns
# for the house election... doing the same thing with John Carter's 2020 vote
# share and info from county clerk websites

tx31_rvotes <- house_elxn %>%
  filter(state == "TX",
         district == 31,
         stage == "gen",
         party == "REPUBLICAN") %>% 
  mutate(rvoteshare = candidatevotes / totalvotes) %>% 
  select(year, office, rvoteshare) %>% 
  bind_rows(tibble(year = 2020,
                   rvoteshare = (145837 + 66858) / 
                     (283792 + 114443),
                   office = "US HOUSE")) %>% 
  full_join(tx31_pres, by = c("year", "office",
                              "rvoteshare"))

# getting data ready for the graph & plotting

test <- tx31_rvotes %>% 
  mutate(office = recode(office,
                         "US HOUSE" = "house",
                         "President" = "pres")) %>% 
  ggplot(aes(year, rvoteshare, color = office)) +
  geom_line() +
  geom_point() +
  geom_hline(yintercept = 0.5, linetype = "dashed",
             color = primary[4]) +
  scale_color_manual(values = monochrome[1:2],
                     labels = c("John Carter",
                                "Republican Presidential \nCandidate")) +
  theme_hodp() +
  labs(x = "",
       y = "Republican Vote Share",
       title = "Republican vote share declining with time in TX-31",
       subtitle = "In both the presidential and congressional races",
       color = "Race") +
  scale_y_continuous(labels = scales::percent_format()) +
  scale_x_continuous(labels = seq(2000, 2020, by = 4),
...
```{r score_plot, fig.width=6, fig.height=4, results="asis"}

# copied this code from Exercise 4 & modified to fit the theme and color scheme

jc %>% 
  ggplot(aes(year, scores, color = voting_score_types)) +
  geom_line() +
  labs(x = "Year",
       y = "Voting Scores",
       title = "John Carter's Voting Scores",
       subtitle = "Dotted vertical lines indicate a change in president",
       color = "Voting Score Type") +
  theme_hodp() +
  ...
  scale_x_continuous(labels = seq(2004, 2020, by = 4),
                     breaks = seq(2004, 2020, by = 4)) +
  guides(color = guide_legend(nrow = 3, byrow = TRUE))

```

```{r score_tables, results="asis"}

# taking median values bc the means would be weird since the numbers are
# drastically different for Bush/Trump vs Obama/Biden years

jc %>% 
  group_by(voting_score_types) %>% 
  summarise(median = median(scores)) %>% 
  kbl(caption = "John Carter's Median Voting Scores",
                  col.names = c("Voting Score Type",
                                "Median")) %>% 
  kable_classic(html_font = "Times",
                            full_width = FALSE)

# getting his presidential support score in 2016... about 56% voted for Trump,
# leaving 44% for Democratic president

jc %>% 
  filter(year == 2016,
         voting_score_types %in% c("Presidential Support (PSS)",
                                   "Party Unity Support (PUS)")) %>% 
  kbl(caption = "Carter voted heavily against Obama in 2016",
                            col.names = c("Voting Score Type",
                                          "Score",
                                          "Year")) %>% 
  kable_classic(html_font = "Times",
                            full_width = FALSE) %>% 
  footnote("In the same year, 44% of TX-31 voted for Hillary Clinton")
...
```{r party_polarization, fig.width=6, fig.height=4, results="asis"}

# modifying code from plot in Exercise 3... storing value for TX-31 R vote share
# in presidential races & Carters's DW-NOMINATE score first

jc_avg_rep <- party %>% 
  filter(state_abbrev == "TX",
         district_code == 31) %>% 
  pull(avg_repub_0816)
...
# putting it on a plot now

party %>% 
  mutate(party_code = as_factor(party_code),
         party_code = recode(party_code, 
                             "100" = "D",
                             "200" = "R",
                             "328" = "I")) %>% 
  ggplot(aes(avg_repub_0816, dwnom1, color = party_code)) +
  geom_point() + 
  scale_color_manual(values = c(primary[4], monochrome[3], primary[5])) +
  guides(color = FALSE) +
  theme_hodp() +
  ...
  geom_hline(yintercept = 0,
             linetype = "dashed") +
  labs(x = "District's average Republican vote share in 2008, 2012, and 2016 presidential elections",
       y = "Member DW-NOMINATE 1 scores",
       title = "Member's voting-based ideology versus district ideology") +
  geom_smooth(method = "lm", se = 0) +
  geom_label(aes(x = jc_avg_rep,
                 y = jc_dwnom1,
                 label = "John \nCarter"))
...
```{r carter_tweets, out.width="100%", out.height="75%"}

# printing out plot with Carter's tweets... doing as a code chunk so that I
# could manually resize and prevent it from loating

knitr::include_graphics("figures/jc_wordcloud.png")
...
```{r tx_avg_dwnom, fig.width=6, fig.height=4, results="asis"}

# creating a plot that shows the average DW-NOMINATE scores across TX Republican
# representatives over time, in relation to Carter's current DW-NOMINATE score

dw %>% 
  filter(state_abbrev == "TX",
         party_code == 200) %>% 
  group_by(congress) %>% 
  summarise(dwnom = mean(nominate_dim1),
            .groups = "drop") %>% 
  ggplot(aes(congress, dwnom)) +
  geom_rect(aes(xmin = 108, xmax = Inf, ymin = 0.471, ymax = Inf),
            alpha = 0.02, fill = "lightgrey") +
  geom_point() +
  geom_smooth(method = "lm", color = monochrome[1]) +
  geom_hline(yintercept = 0.471, color = monochrome[3],
             linetype = "dashed") +
  geom_vline(xintercept = 108, color = monochrome[3],
             linetype = "dashed") +
  geom_text(aes(x = 115, y = 0.4, 
                label = "Carter took \noffice: 108"), color = monochrome[3]) +
  labs(x = "Congress",
       y = "Average DW-NOMINATE score of \nTexas Republicans",
       title = "The average ideology of TX representatives has grown \nmore conservative with time",
       subtitle = "Carter has been less conservative than average during his time in office") +
  geom_text(aes(x = 65, y = 0.49, 
                label = "John Carter's DW-NOMINATE: 0.471"),
            color = monochrome[2]) +
  theme_hodp()
...
![Imam and Hegar Tweets](figures/imam_hegar_wordcloud.png)
